...
if not os.path.isdir("models") or not os.path.isfile("dbt_project.yml"):
    raise FileNotFoundError(
        "Not in a valid root dbt project folder, please run from the folder containing dbt_project.yml"
    )
...
if not validate_json(config, schema=config_schema, validate=True):
    raise ValueError(
        "Invalid config file format, run with flag --configHelp for more information."
    )
...
for event in config.get("events"):
    ...
    if event.get("self_describing_event_aliases") is not None:
        if event.get("self_describing_event_schemas") is None:
            raise ValueError(
                f"Self describing event aliases provided for event name(s) {event.get('event_names')} with no self describing event schemas"
            )
        elif len(event.get("self_describing_event_aliases")) != len(
            event.get("self_describing_event_schemas")
        ):
            raise ValueError(
                f"Length of self describing events schemas and aliases does not match for event name(s) {event.get('event_names')}, please provide an alias for each schema."
            )
    ...
    if event.get("context_aliases") is not None:
        if event.get("context_schemas") is None:
            ...
            raise ValueError(
                f"Length of context schemas and aliases does not match for event name(s) {event.get('event_names')}, please provide an alias for each schema."
            )
...
user_id_column = (
    config.get("users", {}).get("user_id", {}).get("id_column") or "user_id"
)
user_id_sde = (
    config.get("users", {}).get("user_id", {}).get("id_self_describing_event_schema")
)
...
if user_id_sde is not None and user_id_context is not None:
    warnings.warn(
        "Both id_self_describing_event_schema and id_context_schema have been provided, only id_self_describing_event_schema will be used."
    )
...
if len(dupes) > 0:
    raise KeyError(
        f"Configruation leads to duplicate event names, please remove the duplicates and try again. Duplicates: {dupes}"
    )
...
if resolver_file_path != "default":
    ...
    if not os.path.exists(resolver_file_path):
        raise FileNotFoundError(
            f'File {resolver_file_path} not found, to use default Iglu Central please set value to "default".'
        )
    try:
        ...
        print(
            f"Error parsing resolver config at {resolver_file_path}, please ensure this a valid JSON file."
        )
        raise
else:
...
for repo in iglu_resolver_parsed.get("data").get("repositories"):
    ...
    if repo_key is not None and repo_uri[-4:] != "/api":
        raise KeyError(
            f'A private registry uri should end in "/api", {repo_uri} does not, see https://docs.snowplow.io/docs/pipeline-components-and-applications/iglu/iglu-resolver/ for more details.'
        )
...
for i in range(len(event_names)):
    ...
    sde_cols, sde_keys, sde_types, sde_alias = get_cols_keys_types_aliases(
        sde_url, sde_alias, "UNSTRUCT_EVENT_", schemas_list, repo_keys, validate_schemas
    )
    context_cols, context_keys, context_types, context_alias = (
        get_cols_keys_types_aliases(
            context_url,
            context_alias,
            "CONTEXTS_",
            schemas_list,
...
"""
    ...
    filename = os.path.join(
        "models", models_folder, filtered_events_table_name + ".sql"
    )
    verboseprint(
        f"Model content for {filtered_events_table_name}, saving to {filename}:"
    )
...
if user_urls is not None or user_flat_cols is not None:
        ...
        for key_set in user_keys:
            for key in key_set:
                if (
                    ...
                    == re.sub(r"(?<!^)(?=[A-Z])", "_", user_alias).lower()
                ):
                    raise KeyError(
                        f"The user id alias ({user_alias}) exists as a key in one of your contexts (once converted to snakecase), please provide an alternative user id alias in the users section of your config."
                    )
    ...
    meta={{'upsert_date_key': 'latest_collector_tstamp', 'snowplow_optimize': true}}
) }}}}
